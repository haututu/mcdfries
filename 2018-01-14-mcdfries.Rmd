...
In this article I aimed to identify if McDonalds fries were served at the advertised serving size in New Zealand. I only had a small sample so *borrowed power* from a previous sample using a Bayesian prior. Bayesian analyses were conduced using the `BEST` package, a simple to use entry point for Bayesian analyses (although some background would be advisable). All code and data can be found [here](https://github.com/haututu/mcdfries) and there is also a branch with more data so you could follow the code below with more samples to so see how accurate we were (spoiler: pretty accurate).
